...
export const getBookings = async (data: z.input<typeof schema>) => {
	const { businessId, employeeIds, from, to } = schema.parse(data);
	...
	return employees.flatMap(({ id: employeeId, bookings }) =>
		bookings.map(
			({
				id,
				cancelledAt,
