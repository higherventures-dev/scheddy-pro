// features/bookings/services/sendBookingRequestEmail.ts
import 'server-only'
...
export async function sendBookingRequestEmail(
  ...
  const safeTitle = escapeHtml(booking.title || 'Request')
  ...
  const text = [
    ...
    ``,
    `Hi ${fullName},`,
    `Your booking "${booking.title || 'Appointment'}" is requested.`,
    `Starts: ${starts}`,
    `Ends: ${ends}`,
    `Timezone: ${tz}`,
